...
import type { RepairLog, VdeTestResponse } from "@/api/types";

export interface RepairThreadEntry {
  uniqueKey: string;
  type: "repair" | "vde" | "status_change";
  id: number;
  created_at: string;
  person: string;
  duration: number;
...
export function buildVdeSummary(test: VdeTestResponse): string {
  const resultText =
    test.gesamtergebnis === true
      ? "VDE-Test bestanden"
      : "VDE-Test nicht bestanden";

  const details: string[] = [];
  if (test.pruefgeraet_name)
    details.push(`Pruefgeraet: ${test.pruefgeraet_name}`);
...
export function useRepairThread(
  ...
  const lastVdeTestPassed = computed(
    () => sortedVdeTests.value[0]?.gesamtergebnis === true,
  );
  ...
  const threadEntries = computed<RepairThreadEntry[]>(() => {
    ...
    return [...repairEntries, ...vdeEntries].sort(
      (a, b) =>
        new Date(b.created_at).getTime() - new Date(a.created_at).getTime(),
    );
  });
