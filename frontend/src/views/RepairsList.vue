<template>
  <v-container fluid class="pa-4">
    ...
    <v-row class="mb-2">
      ...
      <v-col cols="12" sm="4">
        <v-select v-model="filterStatus" :items="statusFilterOptions" label="Status" variant="outlined"
    ...
    <!-- Repairs table -->
    <v-card>
      <v-data-table :headers="headers" :items="filteredRepairs" :search="search" :loading="loading" :items-per-page="10"
        hover>
        ...
        <template #item.reparatur_dauer="{ item }">
          {{ item.reparatur_dauer }} min
        </template>
    ...
    <v-dialog v-model="deleteDialog" max-width="420">
      <v-card>
        ...
        <v-card-text>
          Möchten Sie die Reparatur <strong>#{{ deleteTargetId }}</strong> wirklich löschen? Diese Aktion kann nicht
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="editForm">
            <v-row dense>
              <v-col cols="12" sm="6">
                <v-text-field v-model="editedItem.datum" label="Datum" type="date" variant="outlined"
                  density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
              ...
              <v-col cols="12" sm="6">
                <v-select v-model="editedItem.reparatur_art" :items="repairTypes" label="Reparaturart"
                  variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field :model-value="editedItem.customer?.vorname" label="Vorname" variant="outlined"
                  density="comfortable" readonly />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field :model-value="editedItem.customer?.nachname" label="Nachname" variant="outlined"
                  density="comfortable" readonly />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field :model-value="editedItem.customer?.telefon" label="Telefon" variant="outlined"
                  density="comfortable" readonly />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field :model-value="editedItem.customer?.email" label="E-Mail" type="email" variant="outlined"
                  density="comfortable" readonly />
              </v-col>
              <v-col cols="12">
                <v-text-field v-model="editedItem.reparatur_sonstiges" label="Sonstiges" variant="outlined"
                  density="comfortable" />
              </v-col>
              <v-col cols="12">
                <v-text-field v-model="editedItem.geraet_art" label="Geräteart" variant="outlined"
                  density="comfortable" />
              </v-col>
              <v-col cols="12">
                <v-textarea v-model="editedItem.defekt_besch" label="Defektbeschreibung" rows="3" variant="outlined"
                  density="comfortable" />
              </v-col>
              <v-col cols="12">
              ...
              <v-col cols="12" sm="6">
                <v-select v-model="editedItem.user_id" :items="userStore.users" item-value="id"
                  :item-title="(u: any) => `${u.vorname} ${u.nachname}`" label="Reparateur" variant="outlined"
                  density="comfortable" clearable :loading="userStore.loading" />
              </v-col>
              <v-col cols="12" sm="6">
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" variant="elevated" @click="saveRepair" :loading="saving">Speichern</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const currentStatusDetailOptions = computed(() => {
  ...
  return REPAIR_STATUS_DETAIL_OPTIONS[editedItem.value.status as keyof typeof REPAIR_STATUS_DETAIL_OPTIONS] || []
})

// Dropdown options from backend
const repairTypes = ref<string[]>([
  'Audio', 'Elektro', 'Fahrrad', 'Möbel', 'PC', 'Spielzeug',
  'Schleifarbeiten', 'Telekom', 'Textil', 'Video', 'Sonstiges'
])
...
watch(
  ...
  (newStatus, oldStatus) => {
    ...
    const allowedDetails = REPAIR_STATUS_DETAIL_OPTIONS[newStatus as keyof typeof REPAIR_STATUS_DETAIL_OPTIONS] || []
    if (editedItem.value.status_detail && !allowedDetails.includes(editedItem.value.status_detail)) {
      editedItem.value.status_detail = ''
    }
  }
)
